...
async function renderNewOrder() {
  ...
  // Supplier selection — fetches products & prices for that supplier
...
async function loadSupplierProducts() {
  ...
  if (!_poSelectedSupplierId) {
    ...
    ingredientsFiltered.innerHTML = `<p class="text-muted" style="padding:16px;text-align:center;margin:0">Sélectionnez un fournisseur pour voir les produits disponibles.</p>`;
    return;
  }
  ...
  if (_poSupplierProducts.length === 0) {
    ingredientsFiltered.innerHTML = `
      <div class="empty-state" style="padding:24px">
        <p>Ce fournisseur n'a pas encore de produits référencés.</p>
        ...
        ${_poItems.map((item, idx) => {
          ...
          return `
            <tr style="border-bottom:1px solid var(--border-color)">
              ...
              <td style="padding:8px;text-align:right">
                <span class="po-readonly-cell" style="display:inline-flex;align-items:center;justify-content:center;min-height:48px;min-width:80px;padding:0 var(--space-4);background:var(--border-default);border-radius:var(--radius-md);color:var(--text-secondary);font:600 var(--text-base)/var(--leading-normal) var(--font-sans)" title="Unité définie par le catalogue fournisseur">${escapeHtml(item.unit || 'unité')}</span>
              </td>
              <td style="padding:8px;text-align:right">
