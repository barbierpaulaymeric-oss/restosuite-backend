...
function renderStepLines() {
  ...
  el.innerHTML = `<ol class="steps-list">${formSteps.map((s, i) =>
    `<li><span style="flex:1">${escapeHtml(s)}</span><span class="ing-remove" role="button" tabindex="0" aria-label="Retirer l'étape" onclick="removeStep(${i})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();removeStep(${i})}"><i data-lucide="x" style="width:14px;height:14px"></i></span></li>`
  ).join('')}</ol>`;
  lucide.createIcons();
}
