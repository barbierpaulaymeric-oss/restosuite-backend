...
function planGate(minPlan) {
  if (!PLAN_ORDER.includes(minPlan)) {
    throw new Error(`planGate: unknown plan '${minPlan}'. Valid plans: ${PLAN_ORDER.join(', ')}`);
  }
  ...
  return (req, res, next) => {
    try {
      ...
      const trialStatus = getAccountStatusById(Number(req.user.id));
      ...
      if (trialStatus.status === 'expired') {
        // HEAD is semantically a read operation — allow it through like GET
