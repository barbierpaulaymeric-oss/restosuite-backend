...
router.get('/', (req, res) => {
  try {
    const items = all('SELECT * FROM equipment_maintenance ORDER BY next_maintenance_date ASC');
    res.json({ items, total: items.length });
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
  ...
  try {
    ...
    const info = run(
      `INSERT INTO equipment_maintenance (equipment_name, equipment_type, location, last_maintenance_date, next_maintenance_date, maintenance_type, provider, cost, status, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [equipment_name, equipment_type || 'autre', location || null,
    ...
    res.status(201).json(get('SELECT * FROM equipment_maintenance WHERE id = ?', [info.lastInsertRowid]));
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
...
router.put('/:id', (req, res) => {
  try {
    ...
    const existing = get('SELECT * FROM equipment_maintenance WHERE id = ?', [id]);
    ...
    res.json(get('SELECT * FROM equipment_maintenance WHERE id = ?', [id]));
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
...
router.delete('/:id', (req, res) => {
  try {
    ...
    const existing = get('SELECT * FROM equipment_maintenance WHERE id = ?', [id]);
    if (!existing) return res.status(404).json({ error: 'Équipement introuvable' });
    run('DELETE FROM equipment_maintenance WHERE id = ?', [id]);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
