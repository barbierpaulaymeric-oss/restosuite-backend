...
router.get('/templates', (req, res) => {
  try {
    const items = all('SELECT * FROM corrective_actions_templates WHERE is_active = 1 ORDER BY category ASC, id ASC');
  ...
  try {
    ...
    const info = run(
      `INSERT INTO corrective_actions_templates
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [category, trigger_condition || null, action_description || null, responsible_role || null,
       deadline_hours != null ? parseInt(deadline_hours, 10) : null,
       escalation_procedure || null, documentation_required || null]
    );
    const item = get('SELECT * FROM corrective_actions_templates WHERE id = ?', [info.lastInsertRowid]);
...
router.put('/templates/:id', (req, res) => {
  try {
    const { id } = req.params;
    const existing = get('SELECT * FROM corrective_actions_templates WHERE id = ?', [id]);
    ...
    const item = get('SELECT * FROM corrective_actions_templates WHERE id = ?', [id]);
    res.json(item);
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
...
router.get('/log', (req, res) => {
  try {
    ...
    let sql = 'SELECT * FROM corrective_actions_log WHERE 1=1';
    const params = [];
  ...
  try {
    ...
    const info = run(
      `INSERT INTO corrective_actions_log
        (template_id, category, trigger_description, action_taken, responsible_person,
         started_at, status, notes, related_record_id, related_record_type)
       VALUES (?, ?, ?, ?, ?, ?, 'en_cours', ?, ?, ?)`,
      [template_id || null, category, trigger_description || null, action_taken || null,
       responsible_person || null, started_at || new Date().toISOString(),
       notes || null, related_record_id || null, related_record_type || null]
    );
    const item = get('SELECT * FROM corrective_actions_log WHERE id = ?', [info.lastInsertRowid]);
    res.status(201).json(item);
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
...
router.put('/log/:id', (req, res) => {
  try {
    const { id } = req.params;
    const existing = get('SELECT * FROM corrective_actions_log WHERE id = ?', [id]);
    ...
    const item = get('SELECT * FROM corrective_actions_log WHERE id = ?', [id]);
    res.json(item);
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
...
router.get('/stats', (req, res) => {
  try {
    const total_en_cours = (get("SELECT COUNT(*) as c FROM corrective_actions_log WHERE status = 'en_cours'") || {}).c || 0;
    const total_termine  = (get("SELECT COUNT(*) as c FROM corrective_actions_log WHERE status = 'terminé'") || {}).c || 0;
    const total_escalade = (get("SELECT COUNT(*) as c FROM corrective_actions_log WHERE status = 'escaladé'") || {}).c || 0;

    const resolutionRows = all(`
      SELECT (julianday(completed_at) - julianday(started_at)) * 24 as hours
      FROM corrective_actions_log
      WHERE status = 'terminé' AND completed_at IS NOT NULL AND started_at IS NOT NULL
    `);
