...
router.post('/', (req, res) => {
  try {
    ...
    const created = get('SELECT * FROM water_management WHERE id = ? AND restaurant_id = ?', [info.lastInsertRowid, rid]);
    try {
      writeAudit({ restaurant_id: rid, account_id: req.user.id ?? null, table_name: 'water_management', record_id: info.lastInsertRowid, action: 'create', old_values: null, new_values: created });
...
router.put('/:id', (req, res) => {
  try {
    ...
    const updated = get('SELECT * FROM water_management WHERE id = ? AND restaurant_id = ?', [id, rid]);
    try {
      writeAudit({ restaurant_id: rid, account_id: req.user.id ?? null, table_name: 'water_management', record_id: id, action: 'update', old_values: existing, new_values: updated });
    } catch (auditErr) { console.error('audit_log write failed:', auditErr); }
    res.json(updated);
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
...
router.delete('/:id', (req, res) => {
  try {
    ...
    try {
      writeAudit({ restaurant_id: rid, account_id: req.user.id ?? null, table_name: 'water_management', record_id: id, action: 'delete', old_values: existing, new_values: null });
    } catch (auditErr) { console.error('audit_log write failed:', auditErr); }
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
